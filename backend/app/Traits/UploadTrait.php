...
trait UploadTrait{
     ...
     */
    public function upload(Request $request, string $field, string $folder, int $file_size_max = 1000,
                           int $img_size_max = 1000, array|string $types = [], bool $thumb = false,
                           int $thumbSize = 120, bool $public = true) : ?string {
     ...
     * @throws \Exception caso alguma validacao barre
     */
    public function uploadFile(?UploadedFile $file, string $field, string $folder, int $file_size_max = 1000,
                               int $img_size_max = 1000, array|string $types = [], bool $thumb = false,
                               int $thumbSize = 120, bool $public = true) : ?string {
        ...
        if(strlen($arq->url) > 0){
            if($thumb) $repo->thumb($arq->url, 120);
            return $arq->url;
        }
